...
const GUID LIBID_AutomationServer = { 0xd70136c, 0xdbc4, 0x4d92, 0xac, 0x2e, 0xb9, 0xc3, 0x65, 0x59, 0x7d, 0x1c };		// ---2
...
HRESULT CMyMath::InitInstance(void) {
	...
	if (m_pITypeInfo == NULL) {
		...
		hr = LoadRegTypeLib(LIBID_AutomationServer, 1, 0/*major/minor version numbers*/, 0x00/*LANG_NEUTRAL*/, &pITypeLib);
		if (FAILED(hr)) {
...
void ComErrorDescriptionString(HWND hwnd, HRESULT hr) {
	...
	if (FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&szErrorMessage, 0, NULL) != 0) {
		...
		swprintf_s(str, TEXT("[Could not find a description for error # %#x.]"), hr);
